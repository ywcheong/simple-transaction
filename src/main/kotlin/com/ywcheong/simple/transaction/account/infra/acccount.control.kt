...
class AccountController(
    ...
    @Operation(
        ...
        ), ApiResponse(responseCode = "400", description = "잘못된 계좌 ID 또는 권한 없음"), ApiResponse(
    ...
    @Operation(
        ...
        security = [SecurityRequirement(name = "bearer-key")],
        parameters = [Parameter(name = "id", description = "계좌 ID (UUID4)", required = true, `in` = ParameterIn.PATH)],
        responses = [ApiResponse(responseCode = "200", description = "폐쇄 성공"), ApiResponse(
            responseCode = "400", description = "잔고가 남아 있거나 권한 없음"
        ), ApiResponse(responseCode = "403", description = "인증 실패")]
    )
    @DeleteMapping("/{id}")
    ...
    @Operation(
        ...
        responses = [ApiResponse(
            ...
            responseCode = "400", description = "잘못된 요청(음수/0 금액, 권한 없음 등)"
        ), ApiResponse(responseCode = "403", description = "인증 실패")]
    )
    ...
    @Operation(
        ...
        responses = [ApiResponse(
            ...
            responseCode = "400", description = "잘못된 요청(음수/0 금액, 잔고 부족, 권한 없음 등)"
        ), ApiResponse(responseCode = "403", description = "인증 실패")]
    )
...
class TransferController(
    ...
    @Operation(
        ...
        responses = [ApiResponse(
            responseCode = "200",
            description = "즉시 송금 성공",
            content = [Content(schema = Schema(implementation = TransferResponse::class))]
        ), ApiResponse(
            responseCode = "202",
            description = "고액 송금 대기",
            content = [Content(schema = Schema(implementation = TransferResponse::class))]
        ), ApiResponse(
            responseCode = "400", description = "잘못된 요청(음수/0 금액, 잔고 부족, 권한 없음 등)"
        ), ApiResponse(responseCode = "403", description = "인증 실패")]
    ...
    @Operation(
        ...
        ), ApiResponse(responseCode = "400", description = "잘못된 이벤트 ID/권한 없음"), ApiResponse(
            responseCode = "403", description = "인증 실패"
        )]
    )
    @GetMapping("/{id}")
